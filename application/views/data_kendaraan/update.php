    ...
    <div class="header-banner-content">
        <div class="container">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="<?php echo site_url('data_kendaraan/update/'.$data->id); ?>" method="post" enctype="multipart/form-data" >
                        <div class="form-row">
                            <div class="form-group col-md-5">
                                <label for="kriteria1">Upload Gambar </label><br/>
                                <?php
                                if ($data->nama_file != '') {
                                    $img_f = $data->path . $data->nama_file;
                                } else {
                                    $img_f = base_url('assets')."/img/noimage.jpg";
                                }
                                ?>
                                <img id="image-preview" src="<?= $img_f ?>" alt="image preview"/>
                                <br/>
                         ...
                         <div class="form-row">
                            <div class="form-group col-md-5">
                                ...
                                <input type="text" class="form-control" name="plat_nomor" id="plat_nomor" value="<?=$data->plat_nomor?>" placeholder="Masukkan Nama" required="true">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-5">
                                <label for="kriteria1">Nama </label>
                                <input type="text" class="form-control" name="nama_kendaraan" id="nama_kendaraan" value="<?=$data->nama?>"  placeholder="Masukkan Nama" required="true">
                            </div>
                         <!--    <div class="form-group col-md-5">
                                <label for="kriteria1">Merek </label>
                                <input type="text" class="form-control" name="merek" id="merek" value="<?=$data->merek?>" placeholder="Masukkan Merek" required="true">
                            </div> -->
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-5">
                                ...
                                <select class="form-control" name="jenis" id="jenis" required="required">
                                    ...
                                    <?php foreach ($this->db->get('tbl_jenis_kendaraan')->result_array() as $key => $value) : ?>
                                        <option value="<?php echo $value['id'] ?>" <?php echo $data->jenis_id== $value['id'] ?"selected":"";?>><?php echo $value['nama'] ?></option>
                                    <?php endforeach ?>
                                </select>
                            </div>
                            <div class="form-group col-md-3">
                                <label for="kriteria1">Tahun </label>
                                <input type="number" class="form-control" name="tahun" id="tahun" value="<?=$data->tahun?>"  placeholder="Masukkan Tahun" required="true">
                            </div>
                        </div>
                        <hr/>
